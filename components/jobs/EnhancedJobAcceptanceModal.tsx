/**
 * Enhanced Job Acceptance Modal Component
...
interface EnhancedJobAcceptanceModalProps {
  visible: boolean;
...
interface OfflineAction {
  id: string;
  type: 'accept' | 'reject' | 'start' | 'complete' | 'update_requirement';
  jobId: string;
  staffId: string;
  data: any;
  timestamp: Date;
...
export default function EnhancedJobAcceptanceModal({
  visible,
  job,
  staffId,
  onClose,
...
}: EnhancedJobAcceptanceModalProps) {
  // Existing state
    ...
    try {
      ...
      for (const action of unsyncedActions) {
        try {
          ...
          switch (action.type) {
            case 'accept':
              await jobAssignmentService.updateJobStatus({
                jobId: action.jobId,
  ...
  const verifyLocation = async (): Promise<boolean> => {
    if (!enableGPSVerification || !job.location.coordinates) return true;
    ...
    try {
      ...
      const verification: LocationVerification = {
        isVerified: distance <= 100, // Within 100 meters
        distance,
  ...
  const calculateAcceptanceProgress = () => {
    let progress = 0;
    let totalSteps = 4; // Base steps: location, requirements, arrival time, final confirmation
    ...
    if (enableRequirementChecking && job.requirements.length > 0) {
    ...
    // Final confirmation (20%) - handled in accept function
    
    setAcceptanceProgress(Math.min(progress, 80)); // Max 80% until final acceptance
  };
  ...
  const handleRequirementNote = (requirementId: string, note: string) => {
    const newNotes = new Map(requirementNotes);
    newNotes.set(requirementId, note);
    setRequirementNotes(newNotes);
  };
  ...
  const estimateArrivalTime = () => {
    ...
    const estimatedMinutes = Math.max(10, Math.round(locationVerification.distance / 500)); // 500m per minute
  ...
  const handleAcceptJob = async () => {
    try {
      ...
      // Requirements check if enabled
      if (enableRequirementChecking) {
    ...
    >
      <View style={styles.container}>
        ...
        >
          ...
          {enableProgressTracking && (
            <View style={styles.progressSection}>
              ...
              <TouchableOpacity onPress={() => setShowProgressDetails(!showProgressDetails)}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          {enableGPSVerification && job.location.coordinates && (
          ...
          {enableRequirementChecking && job.requirements && job.requirements.length > 0 && (
