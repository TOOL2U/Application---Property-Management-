 ...
 * Updated to use jobAssignmentService for consistency with EnhancedStaffJobsView
 */
...
export default function ActiveJobsView() {
  ...
  // State management for active jobs using jobAssignmentService
  ...
  const handleStartJob = async (job: JobAssignment) => {
    try {
      ...
      if (response.success) {
        ...
        await loadActiveJobs(); // Refresh jobs list
      } else {
  ...
  const handlePauseJob = async (job: JobAssignment) => {
    try {
      const response = await jobAssignmentService.updateJobStatus({
        jobId: job.id,
        staffId: currentProfile?.id || '',
        status: 'accepted' // Pause by setting back to accepted
      });
      ...
      if (response.success) {
        ...
        Alert.alert('Error', response.error || 'Failed to pause job');
      }
    } catch (error) {
      console.error('Error pausing job:', error);
      Alert.alert('Error', 'Failed to pause job. Please try again.');
    }
  };
        ...
        >
          ...
          >
            ...
            <View style={styles.scheduleSection}>
              ...
              <Text style={styles.scheduleText}>{formatDate(job.scheduledFor.toDate())}</Text>
            </View>
          </TouchableOpacity>
            ...
            <View style={styles.expandedContent}>
              ...
              <View style={styles.descriptionSection}>
                ...
                <Text style={styles.descriptionText}>
                  {job.description || job.location.accessInstructions || 'No description provided'}
                </Text>
              </View>
                ...
                <View style={styles.contactsContainer}>
                  ...
                  {job.bookingDetails.contactInfo.phone && (
                    ...
                    >
                      ...
                      <Text style={styles.contactText}>{job.bookingDetails.contactInfo.phone}</Text>
                    </TouchableOpacity>
                  )}
                  {job.bookingDetails.contactInfo.email && (
                    <TouchableOpacity
                      style={styles.contactButton}
                      onPress={() => Linking.openURL(`mailto:${job.bookingDetails?.contactInfo?.email}`)}
                    >
                      <User size={16} color="#ffffff" />
                      <Text style={styles.contactText}>{job.bookingDetails.contactInfo.email}</Text>
                    </TouchableOpacity>
                  )}
                </View>
              )}
