...
if (require.main === module) {
  ...
  async function runCommand() {
    try {
      switch (command) {
        case 'verify-all':
          await testService.getStaffVerificationStatus();
          break;
          
        case 'create-test-jobs':
          await testService.createTestJobsForAllStaff();
          break;
          
        case 'cleanup':
          await testService.cleanupTestJobs();
          break;
          
        case 'verify-staff':
          if (args[1]) {
            const result = await testService.verifyStaffUserId(args[1]);
        ...
        case 'get-jobs':
          if (args[1]) {
            await testService.getJobsForStaff(args[1]);
          } else {
